...
_render_aligned: Sequence[
    Callable[[Iterable[Line], Pt, Pt], Iterator[bytes]]
] = [
    _render_left,
    _render_centered,
    _render_right,
...
def _render_text(
    ...
    yield from state
    yield b"%g TL\n" % lead
    yield from _render_aligned[align.value](lines, lead, width)
    yield b"ET\n"
...
def _render_line(start: XY, end: XY, color: RGB) -> Iterator[bytes]:
