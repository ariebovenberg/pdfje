...
from ..common import Char, Func, Ordinal, Pt, add_slots, dictget, first, pipe
from .common import (
...
if TYPE_CHECKING or HAS_FONTTOOLS:
    ...
    class Subset(Font):
        ...
        def new(i: FontID, font: Path) -> Subset:
            ...
            try:
                kern: KerningTable | None = pipe(
                    dictget(ttf["kern"].kernTables[0], 0), scale.__mul__
                )
            except LookupError:
                kern = None
            return Subset(
        ...
        def to_objects(self, obj_id: atoms.ObjectID) -> Iterable[atoms.Object]:
            ...
            yield (
                unicodemap_id,
                atoms.Stream(_to_unicode_map(self.cids.items())),
            )
    ...
    # NOTE: the given widths must be ordered by continuously ascending CID value
    # from 0 onwards
    def _encode_widths(widths: Iterable[GlyphPt]) -> atoms.Array:
        ...
        # TODO: It should be made more efficient in the future,
    ...
    def _encode_cap_height(f: TTFont, scale: float) -> atoms.Int:
        ...
        # See https://fonts.google.com/knowledge/glossary/cap_height
        try:
            v = f["OS/2"].sCapHeight
    ...
    def _to_unicode_map(
        m: Collection[tuple[Ordinal, _CID]]
    ) -> Iterable[bytes]:
