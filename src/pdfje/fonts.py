...
from typing import Collection, Iterable, Iterator, Mapping, Protocol, Sequence
...
from .common import Func, Pt, add_slots, dictget, pipe

FontID = bytes  # unique, internal identifier assigned to a font within a PDF
# unique, case-sensitive name identifying a font family within a PDF
TypefaceName = str
...
class BuiltinFont:
    name: ASCII
    id: FontID
    width: Func[Ordinal, GlyphPt] = field(repr=False)


@add_slots
@dataclass
class Registry:
    builtins: dict[TypefaceName, BuiltinTypeface] = field(default_factory=dict)
    subsets: dict[TTF, EmbeddedSubset] = field(default_factory=dict)
    next_id: Iterator[int] = field(default_factory=count.__call__)

    def to_objects(self, first_id: atoms.ObjectID) -> Iterable[atoms.Object]:
        for sub, i in zip(
            self.subsets.values(),
            count(first_id, step=_OBJS_PER_EMBEDDED_FONT),
        ):
    ...
    def to_resources(self, first_id: atoms.ObjectID) -> atoms.Dictionary:
        return atoms.Dictionary(
            *chain(
                ((b.id, b.to_resource()) for b in self.builtins.values()),
                (
    ...
    def __getitem__(self, f: Typeface) -> Font:
        if isinstance(f, BuiltinTypeface):
            # TODO: nicer
            return self.builtins.setdefault(f.name(), f)
        elif isinstance(f, TTF):
            try:
                return self.subsets[f]
            except KeyError:
                new_subset = self.subsets[f] = EmbeddedSubset.new(
                    b"F%i" % next(self.next_id), f
                )
                return new_subset
        else:
            # TODO: remove this line once we have Font as a proper union
...
class BuiltinTypeface(Typeface):
    main: BuiltinFont
    bold: BuiltinFont | None
    oblique: BuiltinFont | None
    bold_oblique: BuiltinFont | None
    ...
    # TODO: nicer
    def name(self) -> TypefaceName:
        return self.main.name.decode()

    def to_resource(self) -> atoms.Dictionary:
        return atoms.Dictionary(
            (b"Type", atoms.Name(b"Font")),
            (b"Subtype", atoms.Name(b"Type1")),
            (b"BaseFont", atoms.Name(self.main.name)),
    ...
    def width(self, s: str) -> Pt:
        return (
            sum(map(pipe(ord, self.main.width), s)) / TEXTSPACE_TO_GLYPHSPACE
        )

    def kern(self, s: str) -> Kerned:
        # no kerning for builtin fonts for now
        return Kerned((), s)
...
KerningTable = Func[tuple[_Char, _Char], GlyphPt]


# TODO: explain
@dataclass(frozen=True)
class Kerned:
    segments: Sequence[tuple[str, GlyphPt]]
    tail: str

    def space(self) -> Pt:
        return sum(map(itemgetter(1), self.segments)) / TEXTSPACE_TO_GLYPHSPACE


@add_slots
@dataclass(frozen=True)
class EmbeddedSubset:
    id: FontID
    ttf: FontToolsTTF
    charwidth: Func[Ordinal, GlyphPt]
    cids: defaultdict[Ordinal, _CID]
    scale: float  # from glyph units to GlyphPt
    kerning: KerningTable | None

    @staticmethod
    def new(i: FontID, font: TTF) -> EmbeddedSubset:
    ...
    def width(self, s: str) -> Pt:
        return sum(map(pipe(ord, self.charwidth), s)) / TEXTSPACE_TO_GLYPHSPACE
    ...
    def kern(self, s: str) -> Kerned:
        if len(s) <= 1 or self.kerning is None:
            return Kerned((), s)
        else:
            segments: list[tuple[str, GlyphPt]] = []
            prev_split = 0
            for i, (char, next_) in enumerate(pairwise(s), start=1):
                space = self.kerning((char, next_))
                if space:
                    segments.append((s[prev_split:i], space))
                    prev_split = i
            return Kerned(segments, s[prev_split:])
    ...
    def to_objects(self, obj_id: atoms.ObjectID) -> Iterable[atoms.Object]:
        ...
        (
            ...
            cid_gid_map_id,
            file_id,
        ) = range(obj_id + 1, obj_id + _OBJS_PER_EMBEDDED_FONT)
        ...
        yield (
            ...
            atoms.Dictionary(
                ...
                (
                    b"DW",
                    atoms.Real(
                        self.ttf["hmtx"][_REPLACEMENT_GLYPH][0] * self.scale
                    ),
                ),
                (b"W", _encode_widths(map(self.charwidth, self.cids))),
        ...
        yield (
            ...
            atoms.Dictionary(
                ...
                (
                    b"Descent",
                    atoms.Int(round(self.ttf["hhea"].descent * self.scale)),
                ),
...
def _encode_cid_gid_map(
    ttf: FontToolsTTF, cids: Iterable[_CID]
) -> atoms.Stream:
...
def _encode_widths(widths: Iterable[GlyphPt]) -> atoms.Array:
    ...
    # TODO: check if rounding is even needed
    return atoms.Array(
...
courier = BuiltinTypeface(
    BuiltinFont(b"Courier", b"f8", lambda _: 600),
    BuiltinFont(b"Courier-Bold", b"f9", lambda _: 600),
    BuiltinFont(b"Courier-Oblique", b"f10", lambda _: 600),
    BuiltinFont(b"Courier-BoldOblique", b"f11", lambda _: 600),
)

# TODO: actual widths
symbol = BuiltinTypeface(
    BuiltinFont(b"Symbol", b"f12", lambda _: 1000), None, None, None
)  # pragma: no cover
zapf_dingbats = BuiltinTypeface(  # pragma: no cover
    BuiltinFont(b"ZapfDingbats", b"f13", lambda _: 1000), None, None, None
)

BUILTIN_FONTS_BY_NAME: Mapping[TypefaceName, BuiltinTypeface] = {
    f.main.name.decode(): f
    for f in (
        helvetica,
        times_roman,
        courier,
